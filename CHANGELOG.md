...
- **Tile render counter:** `tileUpdatesPerSec` displayed in the Performance panel — counts how often the GL tile render effect actually fires per second. Useful for spotting unnecessary re-renders. Counter lives in Viewport, sampled every second via `setInterval`, passed to Controls as a prop. Code comment and README note explaining why GL and overlay drawing share a single Svelte effect, and when it would be worth splitting.
